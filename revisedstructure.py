...
#        timestr+=f'   {size:.0f}\t{nhdu}\t{nrows}\t{(t1-t0)/1E6:.1f}\t{(t2-t1)/1E6:.1f}\t{(t2-t0)/1E6:.1f}\n'
    ...
    obs  = (t2-t1)/1E6
    total = (t2-t0)/1E6
    names =['Size','N_hdu','N_rows','Load','Create Obsblocks','Total']
    units = ['MB', '', '', 'ms', 'ms', 'ms']
    dtypes = [float, int, int, float,float,float]
    table = Table(data=[size,nhdu,nrows,load,obs,total],
    ...
    if os.path.exists("tab.out"):
        oldtable = Table.read("tab.out",format='ascii')
        t2 = vstack([oldtable,table])
    else:
        t2 = table
    t2.write("tab.out",format='ascii',overwrite=True)
    #t2.pprint_all()
