...
if __name__ == "__main__":
    ...
    for fn in files:
        ...
        if args.baseline:
            ...
            for o in s._obsblock:
                ...
                x = o[0].spectral_axis
                nchan = len(x)
                print(f"DIFF for HDU {j} {nchanl[i]-nchan}",file=sys.stderr)
                cdelt = x[1]-x[0]
                center = x[nchan // 2]
    ...
    base3  = (tb3-tb2)/1E6
    total = (t3-t0)/1E6
    names =['Size','N_hdu','N_rows','N_chan', 'Load','Create_Obsblocks','Baseline_1', 'Baseline_2', 'Baseline_3', 'Total']
    units = ['MB',   '',      '',    '' ,      'ms',      'ms',          'ms',            'ms'      ,   'ms',          'ms' ]
    dtypes = [float, int,    int,    int,      float,     float,            float,          float,         float,        float]
    table = Table(data=[size,nhdu,nrows,nchanl,load,obs,base1,base2,base3,total],
