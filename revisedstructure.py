...
if __name__ == "__main__":
    ...
    i=0
    names =['Size','N_hdu','HDU','N_rows','N_chan', 'Load','Create_Obsblocks','Baseline_1', 'Baseline_2', 'Baseline_3', 'Total']
    units = ['MB',   '',    '',    '',    '' ,      'ms',      'ms',          'ms',            'ms'      ,   'ms',          'ms' ]
    dtypes = [float, int,    int, int,   int,      float,     float,            float,          float,         float,        float]
    ...
    for fn in files:
        ...
        for h in range(1,len(s._hdu)):
            ...
            s._loadlists(fix=args.fix,wcs=args.wcs,hdu=int(hdu))
            ...
            base3  = (tb3-tb2)/1E6
            total = (t3-t0)/1E6
            table.add_row([size,nhdu,hdu,nrows,nchanl,load,obs,base1,base2,base3,total])
        del s
