...
class SDFITSLoad(object):
    ...
    def _loadlists(self,hdu,fix=False,wcs=False,maxspect=1E16):
        ...
        if hdu is not None:
            ...
            with progress_bar:
                ...
                while (n_finished := sum([future.done() for future in futures])) < len(futures):
                    progress_bar.update(
                        overall_progress, completed=n_finished, total=len(futures)
                    )
        ...
        t1 = time.time()
        dt = t1-t0
        print(f"It took {dt:.2f} seconds with {n_processes} processes")

    def _load_one_list(self, j):
        #self._progress[ti] = {"progress": 0, "total": 1}
