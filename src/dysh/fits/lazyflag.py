...
class LazyFlagArray:
    ...
    def _dedup_mask(self, mask_row):
        """Return a deduplicated copy of a 1D mask row.

        Multiple rows with the same flag pattern share the same underlying
        array, reducing memory from O(nrows * nchan) to O(unique_patterns * nchan).
        For VEGAS spurs, this typically reduces 142K row copies to a handful.

        Parameters
        ----------
        mask_row : np.ndarray
            1D boolean mask of length nchan.

        Returns
        -------
        np.ndarray
            A (possibly shared) boolean array with the same content.
        """
        key = mask_row.tobytes()
        pooled = self._mask_pool.get(key)
        if pooled is not None:
            return pooled
        pooled = mask_row.copy()
        pooled.flags.writeable = False  # prevent accidental mutation of shared ref
        self._mask_pool[key] = pooled
        return pooled
    ...
    def or_rows(self, rows, mask):
        ...
        if mask.ndim == 1:
            ...
            if len(rows_array) == self._nrows:
                ...
                return
            # Separate already-modified rows from new rows
            new_rows = []
            for row_idx in rows_array:
                ri = int(row_idx)
                if ri in self._modified:
                    self._modified[ri] = self._modified[ri] | mask
                else:
                    new_rows.append(ri)
            if not new_rows:
                return
            # Batch-read existing flags for all new rows (one fitsio call)
            if self._has_flags_column:
                base = self._read_flags_rows(new_rows)
                for i, ri in enumerate(new_rows):
                    self._modified[ri] = base[i] | mask
            else:
                # Zero-backed: just store the mask directly (shared ref)
                pooled = self._dedup_mask(mask)
                for ri in new_rows:
                    self._modified[ri] = pooled
        elif mask.ndim == 2:
            if mask.shape[0] != len(rows_array):
                raise ValueError(f"mask rows {mask.shape[0]} != number of rows {len(rows_array)}")

            # 2D mask covering all rows: check if all rows are identical -> broadcast
            if len(rows_array) == self._nrows and not self._modified:
                first_row = mask[0]
                if np.all(mask == first_row):
                    self._broadcasts.append(first_row.copy())
                    return

            # Separate already-modified rows from new rows
            new_row_indices = []  # indices into rows_array / mask
            new_row_ids = []  # actual row IDs
            for i, row_idx in enumerate(rows_array):
                ri = int(row_idx)
                if ri in self._modified:
                    self._modified[ri] = self._modified[ri] | mask[i]
                else:
                    new_row_indices.append(i)
                    new_row_ids.append(ri)
            if not new_row_ids:
                return
            # Batch-read existing flags for all new rows (one fitsio call)
            if self._has_flags_column:
                base = self._read_flags_rows(new_row_ids)
                for j, ri in enumerate(new_row_ids):
                    self._modified[ri] = base[j] | mask[new_row_indices[j]]
            else:
                # Zero-backed: deduplicate identical mask rows (common for VEGAS spurs)
                for j, ri in enumerate(new_row_ids):
                    self._modified[ri] = self._dedup_mask(mask[new_row_indices[j]])
        else:
    ...
    def merge_from(self, other):
        ...
        if other._has_flags_column:
            ...
            # This should be rare in practice (additional_channel_mask never has FITS backing).
            ...
            # Batch-read from self's FITS backing for rows we haven't modified yet.
            new_rows = {ri for ri in other._modified if ri not in self._modified}
            if new_rows and self._has_flags_column:
                new_rows_list = sorted(new_rows)
                base = self._read_flags_rows(new_rows_list)
                base_lookup = dict(zip(new_rows_list, base, strict=True))
                for row_idx, row_mask in other._modified.items():
                    if row_idx in self._modified:
                        self._modified[row_idx] = self._modified[row_idx] | row_mask
                    else:
                        self._modified[row_idx] = base_lookup[row_idx] | row_mask
            else:
                for row_idx, row_mask in other._modified.items():
                    if row_idx in self._modified:
                        self._modified[row_idx] = self._modified[row_idx] | row_mask
                    else:
                        # Zero-backed: deduplicate via pool
                        self._modified[row_idx] = self._dedup_mask(row_mask)
    ...
    def __repr__(self):
        ...
        if n_unique:
            parts += f", {n_unique} unique patterns"
        return parts + ")"
