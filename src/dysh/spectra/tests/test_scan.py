...
class TestWeights:
    def test_weights(self, data_dir):
        ...
        # The index file does not define BACKEND or INSTRUME, even though BACKEND is VEGAS
...
class TestFSScan:
    def test_getfs_with_args(self, data_dir):
        ...
        assert abs(nm) <= level
        # Channel range with one signal state.
        assert abs(np.nanmean(diff1[0:5000])) < 7e-8
        ...
        diff = ta_i - ta_f
        assert diff.stats()["mean"].value == pytest.approx(-0.22912714071180673)
        ...
        assert fs.stats()["mean"].value == pytest.approx(0.18852562222605965)
        assert fs.stats()["rms"].value == pytest.approx(5.439287005732594)
        ...
        assert fs.stats()["mean"].value == pytest.approx(0.8382742474775386)
        assert fs.stats()["rms"].value == pytest.approx(25.336939676160352)
