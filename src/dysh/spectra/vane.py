...
class VaneSpectrum(Spectrum):
    ...
    def from_spectrum(
        ...
        spectrum : `~dysh.spectra.spectrum.Spectrum`
            ...
            `~dysh.spectra.vanescan.VaneScan` object, except for the `data` and `flux` attributes
        ...
        vane : None or `~numpy.ndarray`
            ...
            If set, the values will define the `flux` attribute of the `~dysh.spectra.vanescan.VaneScan` object.
        twarm : float or None
            Vane temperature.
        ...
        Returns
        -------
        `~dysh.spectra.vanescan.VaneScan`
            A `~dysh.spectra.vanescan.VaneScan` object.
        """
        if vane is None:
            vane = spectrum.flux
    ...
    def get_tcal(
        ...
        zenith_opacity : float or None
            Opacity towards the zenith at `freq` in nepers. If not provided the value will be retrieved from GBO's
            weather forecast tools (only available at GBO).
        tatm : float or None
            Atmospheric temperature in K. If not provided the value will be retrieved from GBO's
            weather forecast tools (only available at GBO).
        twarm : float or None
            Vane temperature in K. If not provided the value will be retrieved from the object's twarm
            property.
        tbkg : float
            Background temperature in K. By default it adopts the CMB temperature at 3 mm of 2.725 K.

        Returns
        -------
        tcal : float
            Calibration temperature for the vane.
        """
