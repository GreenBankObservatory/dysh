...
for k in [q8stats, q10stats]:
    ...
    # p.pprint_all()
    t[k] = Table(
        names=["data", "mean_load", "median_load", "mean_VMRSS", "#files", "#flags", "skipflags"],
        units=["", "ms", "ms", "MByte", "", "", ""],
        dtype=[str, float, float, float, int, int, str],
    )
    ...
    for i in np.arange(len(p), step=4):
        ...
        vals = [
            p[i]["data"],
            np.mean(p[i : i + 4]["time"]),
            np.median(p[i : i + 4]["time"]),
            np.mean(p[i : i + 4]["VmRSS"]),
            p[i]["#files"],
            p[i]["#flags"],
            p[i]["skipflags"],
        ]
