...
#    parser.add_argument('--baseline','-b', action='store', help='remove baselines',default=None)
    ...
    t1p = np.zeros(len(files))
    t2 = np.zeros(len(files))
    t2p = np.zeros(len(files))
    ...
    i=0
    names =['File', 'Size','N_hdu','HDU','N_rows','N_chan', 'Load','Index', 'Create_Obsblocks','Baseline_1', 'Baseline_2', 'Baseline_3', 'Total']
    units = ['',     'MB',   '',    '',    '',      '' ,    'ms',   'ms',      'ms',             'ms',            'ms'      ,   'ms',       'ms' ]
    dtypes = [str, float,   int,    int,  int,    int,      float,  float,    float,            float,          float,         float,      float]
